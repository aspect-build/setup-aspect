...
// Point `~/.bazelrc` at the Aspect deployment's remote cache and BES, via
// `aspect setup bazelrc --home`. On by default: it is what makes a plain
// `bazel build //...` share a cache between jobs, and it replaces the disk
// cache that used to be the only cache here. Off on Workflows runners, whose
// rc describes the runner's own services and is generated by the other branch.
const remoteCache = !onWorkflowsRunner && core.getBooleanInput('remote-cache')
