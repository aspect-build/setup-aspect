...
| `bazelrc` | "" | Extra lines appended to `~/.bazelrc`. Multiline YAML supported. Append-only and idempotent. Appended after the rc is generated, so these lines win where they set the same flag. Ignored on Workflows runners. |
