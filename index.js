...
function printBazelrc (rcPath) {
  ...
  core.info(fs.readFileSync(rcPath, 'utf8').replace(/^/gm, '  '))
}
 ...
 * Bazel rc so raw `bazel` picks up the same configuration.
 *
 ...
 * works, it warns (raw `bazel` calls won't be configured) but does NOT fail
